...
class SqueezeboxServer(object):
    ...
    def __request(self, line, raw=False, want_reply=True):
        ...
        try:
            ...
            return None
        response = raw_response if raw else urllib.parse.unquote(raw_response)
        if self._debug:
            print_("<<<< \"%s\"" % (response,))
    ...
    def playlist_play(self, path):
        ...
        self.player_request("playlist play %s" % (urllib.parse.quote(path)))

    def playlist_add(self, path):
        self.player_request("playlist add %s" % (urllib.parse.quote(path)), False)

    def playlist_save(self, name):
        self.player_request("playlist save %s" % (urllib.parse.quote(name)), False)
    ...
    def playlist_resume(self, name, resume, wipe=False):
        cmd = ("playlist resume %s noplay:%d wipePlaylist:%d"
               % (urllib.parse.quote(name), int(not resume), int(wipe)))
        self.player_request(cmd, want_reply=False)

    def change_song(self, path):
        """Queue up a song"""
        self.player_request("playlist clear")
        self.player_request("playlist insert %s" % (urllib.parse.quote(path)))
