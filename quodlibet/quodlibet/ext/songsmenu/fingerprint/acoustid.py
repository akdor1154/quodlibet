...
class AcoustidSubmissionThread(threading.Thread):
    ...
    def __send(self, urldata):
        ...
        basedata = urllib.parse.urlencode({
            "format": "xml",
            "client": APP_KEY,
            "user": get_api_key(),
        })

        urldata = "&".join([basedata] + list(map(urllib.parse.urlencode, urldata)))
        obj = io.StringIO()
        ...
        req = urllib.request.Request(self.URL, urldata, headers)
    ...
    for res in json_data.get("results", []):
        ...
        for rec in res.get("recordings", []):
            ...
            for release in rec.get("releases", []):
                ...
                if disc and discs > 1:
                    discnumber = "%d/%d" % (disc, discs)
                else:
                    discnumber = ""

                if track and tracks > 1:
                    tracknumber = "%d/%d" % (track, tracks)
                else:
                    tracknumber = ""
...
class AcoustidLookupThread(threading.Thread):
    ...
    def __process(self, results):
        req_data = []
        req_data.append(urllib.parse.urlencode({
            "format": "json",
            "client": APP_KEY,
            "batch": "1",
        }))
